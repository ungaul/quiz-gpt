...
// Website-specific handlers registry
const WEBSITE_HANDLERS = {
    'kahoot.it': handleKahoot,
    'create.kahoot.it': handleKahoot
};
...
function handleKahoot() {
    // Try to detect Kahoot player/creator experiences
    // Question text
    const questionTitle =
        document.querySelector('[data-functional-selector="question-title"]') ||
        document.querySelector('[data-testid*="questionTitle"]') ||
        document.querySelector('[role="heading"]');

    // Possible containers where answers live
    const answerContainers = [
        ...document.querySelectorAll('[data-functional-selector="answers-list"]'),
        ...document.querySelectorAll('[data-testid*="answers"]'),
        ...document.querySelectorAll('[class*="answers"], [class*="choices"]')
    ];

    // Collect buttons that represent answers
    const answerButtons = new Set();
    answerContainers.forEach(c => {
        c.querySelectorAll('button,[role="button"]').forEach(b => answerButtons.add(b));
    });
    // Direct buttons (some layouts don't have a wrapping answers-list)
    document.querySelectorAll('[data-functional-selector="answer-button"]').forEach(b => answerButtons.add(b));

    const buttons = Array.from(answerButtons).filter(b => isElementUsable(b));

    if (buttons.length === 0) return [];

    const questionText = (questionTitle?.textContent || '').trim() || 'Kahoot question';

    const inputs = buttons.map(btn => {
    ...
    const block =
        questionTitle?.closest('[data-functional-selector="question-container"]') ||
...
function getQuizBlocks() {
    // Try website-specific first
    const handler = WEBSITE_HANDLERS[getCurrentDomain()];
    if (handler) {
        try {
            const siteBlocks = handler();
            if (Array.isArray(siteBlocks) && siteBlocks.length > 0) {
                return siteBlocks; // already in extracted shape
            }
        } catch (_e) {
    ...
    // Generic fallback (returns raw DOM nodes; will be extracted later)
...
function extractBlockData(block) {
    // If already extracted by a site handler, pass through
    if (block && typeof block === 'object' && 'questionText' in block && 'inputs' in block) {
        return block;
    }
    ...
    for (const a of answers) {
        if (a.type === QUESTION_TYPES.TEXT) {
            ...
            a.inputs.forEach((inp, i) => {
                const shouldSelect = sels.includes(String.fromCharCode(65 + i));
                if (shouldSelect) {
                    if (typeof inp.element.click === 'function') {
                        inp.element.click();
                    } else {
                        inp.element.checked = true;
                        inp.element.dispatchEvent(new Event('change', { bubbles: true }));
...
async function processQuiz() {
    const mixedBlocks = getQuizBlocks();
    const blocks = mixedBlocks.map(b => (b && typeof b === 'object' && 'questionText' in b) ? b : extractBlockData(b));
